...
from PIL import Image, ImageDraw  # Import the Image module from Pillow
...
def plot_photo_df(image_path, image_name, data_class):
    """
    Plots the specified image with polygons using the provided data dictionary.

    Parameters:
    image_path (str): Path to the directory containing the images.
    image_name (str): Name of the image file to be plotted.
    data_class (dict): Dictionary containing polygon data.
    """
    ...
    for polygon in polygons:
        ...
        # Create a list of (x, y) tuples for the polygon points
        polygon_points = list(zip(x_coords, y_coords))

        # Draw the polygon on the image
        draw.polygon(polygon_points, outline="green", width=2)
